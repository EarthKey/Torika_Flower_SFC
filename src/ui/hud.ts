...
// スマホ=15秒: 用途が違う。詰まった遊び手は1〜2分で黙って閉じるため、長く置くと
// いちばん助けたい「分からずに離脱する人」の大半が一度も見ないまま去る。
// 🔴 さらに決定的なのが【端末の画面消灯】。Androidの自動ロックは最短15秒、iOSでも30秒が選べる。
// 待ち時間を消灯より長くすると、暗転が出る前に画面そのものが消えてヒントは永久に見られない。
// 早すぎて煩わしくても画面を1回触れば消える＝損失は1タップ。取り逃しは取り返せないので短い側へ倒す。
// （45秒で実装 → 30秒 → 同日、画面消灯の考慮漏れに本人が気づいて15秒へ）
const IDLE_HINT_MS = isTouchDevice ? 15_000 : 120_000
